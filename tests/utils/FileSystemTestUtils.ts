...
import { FileSystemService, LoggerFactory, LogType, YamlUtils } from '../../src/index.js';
...
export class FileSystemTestUtils {
  public static assertSameFolder({
    expectFolder,
    actualFolder,
    excludeFilter,
  }: AssertFoldersParams): void {
    ...
    if (differentContents.length) {
      ...
      expect(
        ...
        `there are differences between folders!. Report:\n\n${report}`,
      ).equals(0);
    }
  }
}
