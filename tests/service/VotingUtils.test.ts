...
import { VotingKeyAccount, VotingUtils } from '../../src/service/index.js';
...
describe('VotingUtils', () => {
  async function assertVotingKey(
    ...
    const votingPublicKey = Convert.uint8ToHex(
      KeyPair.createKeyPairFromPrivateKeyString(privateKey).publicKey,
    );
    ...
    const votingKeyFile = await service.createVotingFile(
      privateKey,
      votingKeyStartEpoch,
      votingKeyEndEpoch,
      unitTestPrivateKeys,
    );
    expect(votingKeyFile.length).toEqual(expectedVotingKeyFile.length);
    const header = votingKeyFile.subarray(0, headerSize);
    const expectedHeader = expectedVotingKeyFile.subarray(0, headerSize);
    expect(header).deep.equal(expectedHeader);
    expect(Convert.uint8ToHex(votingKeyFile)).toEqual(Convert.uint8ToHex(expectedVotingKeyFile));
    const expected: VotingKeyAccount = {
      startEpoch: votingKeyStartEpoch,
      endEpoch: votingKeyEndEpoch,
      publicKey: votingPublicKey,
    };
    expect(service.readVotingFile(votingKeyFile)).deep.equal(expected);
  }
  ...
  it('createVotingFile voting key 1', async () => {
    ...
    const testFile = new Uint8Array(
      readFileSync('./test-fixtures/votingkeys/private_key_tree1.dat'),
    );
  ...
  it.skip('createVotingFile voting key 2', async () => {
    ...
    const privateKey = 'EFE3F0EF0AB368B8D7AC194D52A8CCFA2D5050B80B9C76E4D2F4D4BF2CD461C1';
    const testFile = new Uint8Array(
      readFileSync('./test-fixtures/votingkeys/private_key_tree2.dat'),
    );
    await assertVotingKey(testFile, privateKey, votingKeyStartEpoch, votingKeyEndEpoch);
  });
  ...
  it('createVotingFile voting key 3', async () => {
    ...
    const privateKey = 'EFE3F0EF0AB368B8D7AC194D52A8CCFA2D5050B80B9C76E4D2F4D4BF2CD461C1';
    const testFile = new Uint8Array(
      readFileSync('./test-fixtures/votingkeys/private_key_tree3.dat'),
    );
    await assertVotingKey(testFile, privateKey, votingKeyStartEpoch, votingKeyEndEpoch);
  });
  ...
  it('createVotingFile voting key 4', async () => {
    ...
    const privateKey = 'EFE3F0EF0AB368B8D7AC194D52A8CCFA2D5050B80B9C76E4D2F4D4BF2CD461C1';
    const testFile = new Uint8Array(
      readFileSync('./test-fixtures/votingkeys/private_key_tree4.dat'),
    );
    await assertVotingKey(testFile, privateKey, votingKeyStartEpoch, votingKeyEndEpoch);
  });
  ...
  it('createVotingFile voting key 5', async () => {
    ...
    const testFile = new Uint8Array(
      readFileSync('./test-fixtures/votingkeys/private_key_tree5.dat'),
    );
    await assertVotingKey(testFile, privateKey, votingKeyStartEpoch, votingKeyEndEpoch);
  });
  ...
  const testSignVectorList: VectorData[] = JSON.parse(
    readFileSync('./test-fixtures/votingkeys/2.test-sign.json', 'utf8'),
  );
  VotingUtils.implementations.forEach((implementation) =>
    it(`2.test-sign.json ${implementation.name} `, async () => {
      for (const vector of testSignVectorList) {
        ...
        expect(await implementation.createKeyPairFromPrivateKey(privateKey)).deep.equal(keyPair);
        const resolvedSignature = await implementation.sign(
          keyPair,
          Convert.hexToUint8(vector.data),
        );
        expect(Convert.uint8ToHex(resolvedSignature)).toEqual(vector.signature);
      }
    }),
  );
});
