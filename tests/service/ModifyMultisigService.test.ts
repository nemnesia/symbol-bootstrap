...
import { LoggerFactory, LogType, Utils } from '../../src/index.js';
import { ConfigPreset } from '../../src/model/index.js';
import {
...
import { StdUtils } from '../utils/StdUtils.js';
...
describe('ModifyMultisigService', () => {
  ...
  const target = 'target/test-output/testnet-dual';
  ...
  const commonStub = async (
    ...
    sandbox.stub(TransactionUtils, <any>'getRepositoryFactory');
    sandbox
      .stub(TransactionUtils, <any>'getMultisigAccount')
      .returns(Promise.resolve(multisigAccount));
  ...
  it('Converts regular account to multisig, adding single cosignatory', async () => {
    ...
    expect(tx.type).toEqual(TransactionType.MULTISIG_ACCOUNT_MODIFICATION);
    expect(tx.addressAdditions.length).toEqual(1);
    expect(tx.addressDeletions.length).toEqual(0);
    expect(tx.minRemovalDelta).toEqual(1);
    expect(tx.minApprovalDelta).toEqual(1);
  });

  it('Converts regular account to multisig, adding multiple cosignatories', async () => {
    StdUtils.in(['\n']); // for addressDeletions

    const tx = await commonStub(
      [cosigner1, cosigner2].map((c) => c.address.plain()).join(','),
      undefined,
      1,
      1,
      undefined,
    ...
    expect(tx.type).toEqual(TransactionType.MULTISIG_ACCOUNT_MODIFICATION);
    expect(tx.addressAdditions.length).toEqual(2);
    expect(tx.addressDeletions.length).toEqual(0);
    expect(tx.minRemovalDelta).toEqual(1);
    expect(tx.minApprovalDelta).toEqual(1);
  });
  ...
  it('Removes a cosignatory from the current multisig account', async () => {
    ...
    expect(tx.type).toEqual(TransactionType.MULTISIG_ACCOUNT_MODIFICATION);
    expect(tx.addressDeletions.length).toEqual(1);
    expect(tx.addressAdditions.length).toEqual(0);
    expect(tx.minRemovalDelta).toEqual(-1);
    expect(tx.minApprovalDelta).toEqual(-1);
  });
  ...
  it('Throws error when new minApproval is larger than the total number of cosignatories ', async () => {
    ...
    try {
      ...
      expect(Utils.getMessage(err)).toEqual(
  ...
  it('Throws error while cosignatory to be added already a cosignatory.', async () => {
    ...
    try {
      ...
      expect(Utils.getMessage(err).startsWith('Cannot add cosignatory!')).toBe(true);
    }
  });
  ...
  it('Throws error while cosignatory to be removed is not an actual cosignatory.', async () => {
    ...
    try {
      ...
      expect(Utils.getMessage(err).startsWith('Cannot remove cosignatory!')).toBe(true);
    }
  });
});
