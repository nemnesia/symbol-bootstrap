...
import { LoggerFactory, LogType, OSUtils, RuntimeService, Utils } from '../../src/index.js';
...
describe('RuntimeService', async () => {
  ...
  it('exec when invalid', async () => {
    try {
      ...
      expect(true).toEqual(false); //Should fail!!
    } catch (e) {
      const error = e as any;
      expect(Utils.getMessage(e).indexOf('wrong!')).not.toEqual(-1);
      expect(error.stderr.indexOf('wrong!')).not.toEqual(-1);
      expect(error.stdout).toEqual('');
      expect(error.cmd).toEqual('wrong!');
      expect(error.code).toEqual(OSUtils.isWindows() ? 1 : 127);
    }
  });

  it('exec when invalid ignore error', async () => {
    const result = await service.exec('wrong!', true);
    expect(result.stderr.indexOf('wrong!')).not.toEqual(-1);
