...
describe('Announce Service', () => {
  ...
  const params = {
    ...
    ready: true,
    target: 'target/test-output/announce-service-test',
    password,
    reset: false,
    offline: true,
  ...
  const mainPublicKey = Account.createFromPrivateKey(
    params.customPresetObject.nodes[0].mainPrivateKey,
    networkType,
  ).publicKey;
  ...
  const cosigner1 = Account.createFromPrivateKey(
    '41C0163B6A057A4E7B6264AC5BB36C44E0245F8552242BF6A163617C4D616ED3',
    networkType,
  );
  const cosigner2 = Account.createFromPrivateKey(
    '2FBDC1419F22BC049F6E869B144778277C5930D8D07D55E99ADD2282399FDCF5',
    networkType,
  );
  ...
  const stubCommon = (
    ...
    sandbox.stub(RemoteNodeService.prototype, 'getBestRepositoryInfo').callsFake(() =>
      Promise.resolve({
        repositoryFactory: {
          getNetworkType() {
            return of(networkType);
          },
          createTransactionRepository: sandbox.stub(),
          createReceiptRepository: sandbox.stub(),
          getEpochAdjustment() {
    ...
    sandbox.stub(announceService, <any>'getAccountInfo').returns(
      Promise.resolve({
  ...
  it('Main account regular - Announces simple transaction when single transaction given', async () => {
    ...
    expect(addresses!.nodes![0]!.main.publicKey).toEqual(mainPublicKey);

    stubCommon(networkType, epochAdjustment, currencyMosaicId, networkGenerationHash);
    const tsAnnounce = sandbox
      .stub(TransactionService.prototype, 'announce')
      .returns(of({} as Transaction));
    const announceSimple = sandbox.spy(announceService, <any>'announceSimple');
    ...
    expect(announceSimple.called).toBe(true);
    expect(tsAnnounce.calledOnceWith(match({ signerPublicKey: mainPublicKey }), match.any)).toBe(
      true,
    );
  });
  ...
  it('Main account regular- Announces aggregate complete when multiple transactions given', async () => {
    ...
    const tsAnnounce = sandbox
      .stub(TransactionService.prototype, 'announce')
      .returns(of({} as Transaction));
    const announceAggregateComplete = sandbox.spy(
      announceService,
      <any>'announceAggregateComplete',
    );
    await announceService.announce(
      url,
      maxFee,
    ...
    expect(announceAggregateComplete.called).toBe(true);
  ...
  it('Main account multisig - Announces aggregate complete when single transaction and all required cosignatures given', async () => {
    ...
    sandbox.stub(TransactionUtils, <any>'getMultisigAccount').returns(
      Promise.resolve({
        minApproval: 2,
      }),
    );
    sandbox
      .stub(announceService, <any>'getMultisigBestCosigner')
      .callsFake((multisigAccountInfo, cosigners) => {
    ...
    expect(announceAggregateComplete.called).toBe(true);
    expect(
      tsAnnounce.calledOnceWith(match({ signerPublicKey: bestCosigner.publicKey }), match.any),
    ).toBe(true);
  });
  ...
  it('Service provider account regular - Announces aggregate bonded', async () => {
    ...
    stubCommon(networkType, epochAdjustment, currencyMosaicId, networkGenerationHash);
    sandbox
      .stub(accountResolver, 'resolveAccount')
      .returns(Promise.resolve(serviceProviderAccount));

    const tsAnnounce = sandbox
      .stub(TransactionService.prototype, 'announce')
      .returns(of({} as Transaction));
    const tsAnnounceBonded = sandbox
      .stub(TransactionService.prototype, 'announceAggregateBonded')
      .returns(of({} as AggregateTransaction));
    const announceAggregateBonded = sandbox.spy(announceService, <any>'announceAggregateBonded');
    const createBonded = sandbox.spy(AggregateTransaction, 'createBonded');
    await announceService.announce(
      url,
      maxFee,
      useKnownRestGateways,
    ...
    expect(announceAggregateBonded.called).toBe(true);
    expect(
      tsAnnounce.calledOnceWith(
        match({ signerPublicKey: serviceProviderAccount.publicKey }),
        match.any,
      ),
    ).toBe(true);
    expect(
      tsAnnounceBonded.calledOnceWith(
        match({ signerPublicKey: serviceProviderAccount.publicKey }),
        match.any,
      ),
    ).toBe(true);
  ...
  it('Service provider account multisig - Announces aggregate bonded', async () => {
    ...
    sandbox.stub(accountResolver, 'resolveAccount').returns(Promise.resolve(bestCosigner));
    const tsAnnounce = sandbox
      .stub(TransactionService.prototype, 'announce')
      .returns(of({} as Transaction));
    const tsAnnounceBonded = sandbox
    ...
    expect(
      tsAnnounceBonded.calledOnceWith(
        match({ signerPublicKey: bestCosigner.publicKey }),
        match.any,
      ),
    ).toBe(true);
    expect(
