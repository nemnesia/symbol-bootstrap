...
import { ConfigPreset, LoggerFactory, LogType, YamlUtils } from '../../src/index.js';
import { ConfigLoader, Preset, RemoteNodeService } from '../../src/service/index.js';
...
const presetData: ConfigPreset = new ConfigLoader(logger).mergePresets(
  sharedPreset,
  networkPreset,
  customPresetObject,
);
...
describe('RemoteNodeService', () => {
  ...
  it('getRestUrls online', async () => {
    sandbox.stub(RemoteNodeService.prototype, 'createNodeApiRestClient').callsFake(() => {
      return {
        getNodes(filter: NodeFilter, limit: number) {
          expect(filter).toEqual(presetData.statisticsServiceRestFilter);
          expect(limit).toEqual(presetData.statisticsServiceRestLimit);
          return list;
        },
  ...
  it('getRestUrls offline', async () => {
    ...
    const urls = await service.getRestUrls();
    expect(urls).deep.equal(['http://staticRest1:3000', 'https://staticRest2:3001']);
  });
  it('getPeerInfos online', async () => {
    sandbox.stub(RemoteNodeService.prototype, 'createNodeApiRestClient').callsFake(() => {
      return {
        getNodes(filter: NodeFilter, limit: number) {
          expect(presetData.statisticsServicePeerFilter).toEqual('');
          expect(filter).toEqual(undefined);
          expect(limit).toEqual(presetData.statisticsServicePeerLimit);
          return list;
        },
      } as unknown as NodeApi;
    });
  ...
  it('getPeerInfos offline', async () => {
    sandbox.stub(RemoteNodeService.prototype, 'createNodeApiRestClient').callsFake(() => {
      return {
        getNodes(filter: NodeFilter, limit: number) {
          expect(filter).toEqual(presetData.statisticsServicePeerFilter);
          expect(limit).toEqual(presetData.statisticsServicePeerLimit);
          return list;
        },
      } as unknown as NodeApi;
