...
import { Assembly, CustomPreset, LoggerFactory, LogType, Utils } from '../../src/index.js';
import { ConfigService, CryptoUtils, Preset } from '../../src/service/index.js';
// Local test utils
...
describe('ConfigService', () => {
  it('ConfigService bootstrap run with optin_preset.yml', async () => {
    await new ConfigService(logger, {
      ...
      target: 'target/test-output/ConfigService.test.optin',
      customPreset: './test-fixtures/optin_preset.yml',
    }).run();
  });
  ...
  it('ConfigService bootstrap in custom preset run with override-currency-preset.yml', async () => {
    await new ConfigService(logger, {
      ...ConfigService.defaultParams,
      reset: true,
      target: 'target/test-output/ConfigService.test.custom',
  ...
  it('ConfigService testnet assembly', async () => {
    await new ConfigService(logger, {
      ...ConfigService.defaultParams,
      reset: true,
      target: 'target/test-output/ConfigService.test.testnet',
  ...
  it('ConfigService mainnet assembly', async () => {
    await new ConfigService(logger, {
      ...ConfigService.defaultParams,
      reset: true,
      target: 'target/test-output/ConfigService.test.mainnet',
  ...
  it('ConfigService bootstrap default', async () => {
    const configResult = await new ConfigService(logger, {
      ...
      reset: true,
      password: '1111',
      target: 'target/test-output/bootstrap',
      preset: Preset.bootstrap,
    }).run();

    expect(configResult.addresses.mosaics?.length).toEqual(2);
    expect(configResult.addresses.mosaics?.[0]?.accounts.length).toEqual(5);
    expect(configResult.addresses.mosaics?.[1]?.accounts.length).toEqual(2);
    ...
    expect(configResult.addresses).deep.equal(configResultUpgrade.addresses);

    expect(CryptoUtils.removePrivateKeys(configResultUpgrade.presetData)).deep.equal(
      CryptoUtils.removePrivateKeys(configResult.presetData),
    );
  });

  it('ConfigService custom network yml file', async () => {
    const target = 'target/test-output/ConfigService.test.customNetwork';
    const workingDir = 'test-fixtures/customNetwork';
    await new ConfigService(logger, {
      ...ConfigService.defaultParams,
  ...
  it('ConfigService custom network yml file invalid seed folder', async () => {
    ...
    try {
      ...
      expect(Utils.getMessage(e)).toEqual(
        `${join('test-fixtures', `customNetwork`, 'nemesis-seed-invalid')} folder does not exist`,
      );
    }
  });
  ...
  it('ConfigService bootstrap repeat', async () => {
    const configResult = await new ConfigService(logger, {
      ...ConfigService.defaultParams,
      reset: true,
      target: 'target/test-output/ConfigService.bootstrap.repeat',
      preset: Preset.bootstrap,
      customPreset: './test-fixtures/repeat_preset.yml',
    }).run();

    const assertRepeatedService = (expectedCount: number, services: any[] | undefined) => {
      expect(services!.length).toEqual(expectedCount);

      services?.forEach((service) => {
        Object.values(service).forEach((value) => {
          expect((value + '').indexOf('index'), `'${value}' contains index!`).toEqual(-1);
        });
      });
    };
  ...
  it('ConfigService resolve nemesis balances', async () => {
    ...
    const configResult = await new ConfigService(logger, {
      ...ConfigService.defaultParams,
      reset: true,
      target: 'target/test-output/ConfigService.bootstrap.nemesis.balances',
      preset: Preset.bootstrap,
      assembly: Assembly.dual,
      customPresetObject: customPresetObject,
    }).run();
    const presetData = configResult.presetData;
    expect(presetData.nemesis).deep.equal(
      {
        mosaics: [
          {
            name: 'currency',
    ...
    expect(presetData.nemesis.mosaics.length).toEqual(2);
    presetData.nemesis.mosaics.forEach((mosaic) => {
      expect(_.sumBy(mosaic.currencyDistributions, (d) => d.amount)).toEqual(mosaic.supply);
    });

    expect(configResult.addresses.mosaics).deep.equal(
      [
        {
          id: '113DFC906359F64D',
          name: 'currency',
  ...
  it('ConfigService resolve nemesis balances no extra generated accounts', async () => {
    ...
    const configResult = await new ConfigService(logger, {
      ...ConfigService.defaultParams,
      reset: true,
      target: 'target/test-output/ConfigService.bootstrap.nemesis.balances.no.extra.gen.accounts',
      preset: Preset.bootstrap,
      assembly: Assembly.dual,
      customPresetObject: customPresetObject,
    }).run();
  ...
  it('ConfigService invalid nemesis balances, larger than supply', async () => {
    ...
    try {
      await new ConfigService(logger, {
        ...ConfigService.defaultParams,
        reset: true,
        target: 'target/test-output/ConfigService.bootstrap.invalid.nemesis.balances',
        preset: Preset.bootstrap,
        assembly: Assembly.dual,
        customPresetObject: customPresetObject,
      }).run();
      ...
      expect(Utils.getMessage(e)).toEqual(
        "Mosaic currency's fixed distributed supply 1000005 is grater than mosaic total supply 1000000",
      );
    }
  });
});
