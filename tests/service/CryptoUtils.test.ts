...
import { CryptoUtils } from '../../src/service/CryptoUtils.js';
...
describe('CryptoUtils', () => {
  it('encrypt decrypt', () => {
    ...
    expect(encryptedObjectJson).not.deep.equal(object);

    expect(CryptoUtils.encryptedCount(object)).toEqual(0);
    expect(CryptoUtils.encryptedCount(encryptedObject)).toEqual(7);

    const decryptedObject = CryptoUtils.decrypt(JSON.parse(encryptedObjectJson), password);
    expect(decryptedObject).deep.equal(object);

    const decryptedObject2 = CryptoUtils.decrypt(object, password);
    expect(decryptedObject2).deep.equal(object);
  });
  ...
  it('encrypt decrypt invalid password', () => {
    ...
    try {
      ...
      expect(Utils.getMessage(e)).toEqual('Value could not be decrypted!');
    }
  });
  ...
  it('it removes private keys', () => {
    ...
    expect(noPrivateKeyObject).deep.equal(restrictedObject);
  });
